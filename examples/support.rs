...
//! Shared display helpers for http-handle examples.
//!
//! Animated spinner + checkmark output. Each example imports this
//! module via `#[path = "support.rs"] mod support;` so the helpers
//! never have to be re-exported through the library crate.
...
/// Print a footer (blank line). Mirrors `noyalib`'s helper for
/// examples that prefer an explicit footer over `summary`.
pub(crate) fn footer() {
    println!();
}
...
/// Run a task, then print additional detail lines beneath the
/// checkmark.
...
/// Run a task that may fail. Shows checkmark on success, cross on
/// failure with the error rendered next to the label.
...
/// Show a summary line. Pass the number of operations completed.
