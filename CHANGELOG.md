...
- **Multi-thread async runtime** — new `high-perf-multi-thread` feature flag and `perf_server::start_high_perf_multi_thread(server, limits, worker_threads)` entry point. Builds a Tokio multi-thread runtime internally so callers don't need to add `rt-multi-thread` to their tokio features. `worker_threads = None` defaults to logical CPU count; `Some(n)` pins worker count for reproducible benchmarking or container CPU caps. `bench` example (`examples/bench.rs`) exposes the new mode as `HTTP_HANDLE_MODE=high-perf-mt` with optional `HTTP_HANDLE_WORKERS=N`. Honest comparison numbers and tradeoff guidance in `docs/PERFORMANCE.md`: with the v0.0.5 response cache + coalesced write, `high-perf-mt` overtakes sync on Apple Silicon and pulls ahead by 1.6× on Linux/arm64 (180 k req/s).
...
- **`dhat` example** (`examples/dhat.rs`) — `cargo run --release --example dhat --features high-perf` (or `./scripts/example.sh dhat --release`) writes `dhat-heap.json` for offline allocation analysis.
- **30 one-word examples** mirroring `noyalib/examples`: `hello`, `builder`, `request`, `response`, `errors`, `policies`, `pool`, `shutdown`, `keepalive`, `language`, plus one per Cargo feature (`async`, `batch`, `streaming`, `optimized`, `observability`, `http2`, `http3`, `perf`, `multi`, `autotune`, `ratelimit`, `tenant`, `tls`, `auth`, `config`, `enterprise`), and tooling (`full`, `all`, `bench`, `dhat`). All share `examples/support.rs` for animated spinner output. `scripts/example.sh <name>` auto-resolves the required Cargo features so users don't need to remember the `--features` flag per demo. Capability → example matrix in `docs/EXAMPLES.md`.
